...
const VideoCard = ({ thumbnail, title, uploadedAt, views, owner, videoId }) => {
   const webpThumbnail = thumbnail.slice(0, -4) + ".webp";
   const [ownerData, setOwnerData] = useState([]);
   const fetchOwnerData = async () => {
      try {
         const response = await fetch(`/api/v1/user/${owner}`);
         const data = await response.json();

         if (data.success) setOwnerData(data.data);
   ...
   return (
      <div className="flex flex-col space-y-4 w-[435px] h-[350px] mx-2 my-2  hover:bg-gray-100 rounded-xl p-2 overflow-hidden">
         {/*          Thumbnail */}
