...
export const callEmailService = serviceHandler(async (userMeta, type) => {
   const channel = getChannel();

   if (!channel) {
      authServiceLogger.error("RabbitMQ channel not initialized");
      throw new ApiError(500, "Email service is not available");
   }
   ...
   authServiceLogger.info("Email sent", {
      userId: userMeta._id,
...
export const registerUser = serviceHandler(
   async (
      ...
      await callEmailService(createdUser, "registration");
...
export const confirmEmail = serviceHandler(async (userMeta) => {
   ...
   await callEmailService(user, "welcome");
...
export const forgotPassword = serviceHandler(async (email) => {
   ...
   await callEmailService(user, "resetPassword");
});
...
export const loginUser = serviceHandler(async (email, password) => {
   ...
   if (!isPasswordCorrect)
      throw new ApiError(401, "Incorrect Password", { email });
