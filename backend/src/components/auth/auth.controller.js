...
const googleLogin = asyncHandler(async (req, res) => {
   ...
   authLogger.info(`[Request] ${req.id} Google login attempt`, {
      route: "POST /auth/google",
   });

   const googleProfile = await googleAuthService.verifyToken(tokenId);

   authLogger.debug(`[Request] ${req.id} Google token verified`, {
      email: googleProfile.email,
   });

   const user = await googleAuthService.findOrCreateUser(googleProfile);

   authLogger.debug(`[Request] ${req.id} Google login successful`, {
      userId: user._id,
      email: user.email,
   });
   ...
   authLogger.info(`[Request] ${req.id} Tokens generated`, {
      userId: user._id,
   });
