...
const findUserByToken = serviceHandler(async (token, tokenType) => {
   ...
   let decodedToken;
   if (tokenType === "cft") decodedToken = jwt.verify(token, ENV.CONFIRMATION_TOKEN_SECRET); 
   else if (tokenType === "rft") decodedToken = jwt.verify(token, ENV.REFRESH_TOKEN_SECRET);
...
export const isConfirmationTokenValid = serviceHandler(async (confirmationToken) => {
   const user = await findUserByToken(confirmationToken, "cft");

   if (user.isEmailConfirmed) return { status: false, message: "Email is already confirmed" }
   if (!user) return { status: false, message: "Invalid Confirmation Token"}

   return { status: true, message: "Confirmation Token is valid", userMeta: user}
});
...
export const resetPassword = serviceHandler(async (token, newPassword) => {
   ...
   log(`Reset password for user: ${user.username} from ${user.password} to ${newPassword} `)
});
...
export const changePassword = serviceHandler(
	async (userId, oldPassword, newPassword) => {
      ...
      log(`Changed Password for user: ${user.username} from {${oldPassword}} to {${newPassword}}`)
	},
);
...
export const updateAccountDetails = serviceHandler(
	async (userId, fullName, username) => {
      ...
      log(`Updated account details for user: ${user.username} to ${fullName}, ${username} `)
		return user;
	},
);
...
export const updateUserAvatar = serviceHandler(
	async (userid, avatarLocalPath) => {
      ...
      log(`Updated avatar for user: ${user.username} from ${user.avatar} to ${updatedUser.avatar}`)
		return updatedUser;
	},
);
...
export const updateCoverAvatar = serviceHandler(
	async (userid, coverLocalPath) => {
      ...
      log(`Updated cover image for user: ${user.username} from ${user.coverImage} to ${updatedUser.coverImage} `)
		return updatedUser;
	},
);
